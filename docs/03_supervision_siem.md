...
| Hôte Docker — topologie réseau (`backend`, `authentik_proxy`, `authentik_internal`) | `docker network inspect <nom> \| grep Internal` | Authentik tournant sur la même VM (plus de LAN egress à filtrer), l'isolation est désormais structurelle : les trois réseaux doivent rester `"Internal": true`. Une dérive (quelqu'un retire ce flag) réintroduirait un chemin d'egress WAN/LAN — vérification périodique, pas seulement à l'installation. |
